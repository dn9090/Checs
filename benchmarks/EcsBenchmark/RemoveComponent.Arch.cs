...
namespace EcsBenchmark
{
	public partial class RemoveComponent
	{
		...
		public void Arch_Setup()
		{
			...
			Arch_world.Reserve(new[] { typeof(Unmanaged.Component1) }, Arch_entities.Length);
			Arch_world.Reserve(Array.Empty<Type>(), Arch_entities.Length);
			
			for(int i = 0; i < Arch_entities.Length; ++i)
				Arch_entities[i] = Arch_world.Create<Unmanaged.Component1>();
		}
		...
		public void Arch(int entityCount)
		{
			for(int i = 0; i < entityCount; ++i)
				Arch_world.Remove<Unmanaged.Component1>(in Arch_entities[i]);
		}
	}
}
